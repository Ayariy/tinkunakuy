<div class="">
    ...
    <div class="row mb-1 ">
        ...
        <a href="{{route('admin.miembros.cargo.create',[],false)}}"
                ...
                <tr wire:key="item-{{$cargo->idcargo }}">
                    ...
                    <td>
                        <div class="d-flex flex-column  flex-md-row justify-content-between">
                            {{$cargo->cargotrans[0]->cargoTrans}}

                            <x-adminlte-button class="ml-md-2" type="submit" theme="primary"
                                label="{{__('messages.editar')}}" icon="fas fa-edit" data-toggle="modal"
                                data-target="#modalCargo{{$cargo->cargotrans[0]->idCargoTrans}}" />
                        </div>

                        @livewire('admin.miembros.editar-cargo', ['cargotrans' => $cargo->cargotrans[0]],
                        key('item-'.$cargo->cargotrans[0]->idCargoTrans))


                    </td>
                    <td>
                        <div class="d-flex flex-column  flex-md-row justify-content-between">
                            {{$cargo->cargotrans[1]->cargoTrans}}

                            <x-adminlte-button class="ml-md-2" type="submit" theme="primary"
                                label="{{__('messages.editar')}}" icon="fas fa-edit" data-toggle="modal"
                                data-target="#modalCargo{{$cargo->cargotrans[1]->idCargoTrans}}" />

                        </div>

                        @livewire('admin.miembros.editar-cargo', ['cargotrans' => $cargo->cargotrans[1]],
                        key('item-'.$cargo->cargotrans[1]->idCargoTrans))
                    </td>
                    <td>
                        <div class="d-flex flex-column  flex-md-row justify-content-between">
                        ...
                        @livewire('admin.miembros.editar-cargo', ['cargotrans' => $cargo->cargotrans[2]],
                        key('item-'.$cargo->cargotrans[2]->idCargoTrans))
                    </td>
                    <td>
                        <x-adminlte-button wire:click="$emit('alertDelete', {{$cargo->idcargo}})" type="submit"
